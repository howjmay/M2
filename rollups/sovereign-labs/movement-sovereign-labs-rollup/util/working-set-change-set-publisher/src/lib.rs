...
/// This is an implementation of remote cache which will be back of TransactionDataCache
        ...
        for (account_address, account_changes) in change_set.accounts().iter() {
            ...
            for (path, op) in modules.iter() {
                ...
                match op {
                    Op::New(data) => {
                        self.remote_cache
                            .set(&AccessPathWrapper::new(ap), data, &mut working_set);
                    }
                    Op::Modify(data) => {
                        self.remote_cache
                            .set(&AccessPathWrapper::new(ap), data, &mut working_set);
                    ...
                    Op::Delete => {
                        self.remote_cache
                            .remove(&AccessPathWrapper::new(ap), &mut working_set);
                    }
                }
            }
            ...
            for (path, op) in resources.iter() {
                let ap =
                    AccessPath::resource_access_path(*account_address, path.clone().to_owned())?; // struct tag is actually somewhat large so avoiding clone may be useful
                match op {
                    Op::New(data) => {
                        self.remote_cache
                            .set(&AccessPathWrapper::new(ap), data, &mut working_set);
